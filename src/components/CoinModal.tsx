...
import { addFavorites, removeFavorites } from "../state/favoritesSlice"
...
export default function CoinModal({ id, name, symbol, image, supply, marketCap, vwap24Hr, changePercent24Hr, priceUsd, onClose }: CoinModalProps) {
        ...
        <>
            <div className="grid grid-cols-12 grid-rows-7 lg:h-10/12 h-9/12 lg:w-10/12 w-11/12 place-self-center bg-gray-900/85 rounded-md bg-clip-padding backdrop-filter backdrop-blur-md border border-gray-100 fixed inset-0">
                <div className="flex col-span-9 col-start-2 w-5/12 place-items-center lg:gap-5 gap-2 ">
                    ...
                    <p className="text-white text-xl md:text-4xl drop-shadow-md drop-shadow-black">{name}</p>
                    <p className="text-white text-xl md:text-4xl drop-shadow-md drop-shadow-black">{symbol}</p>
                </div>

                <div className="flex flex-col md:grid grid-cols-2 gap-4 width-40 col-span-10 row-span-5 col-start-2 row-start-2 place-self-center bg-black/40 rounded-md p-6">
                    <div>
                ...
                <div className="flex col-start-11 col-span-2 place-items-center justify-center items-center">
                    <img src="public/xmark-svgrepo-com.svg" alt="Close" onClick={onClose} className=" bg-red-500 rounded-full max-w-10 drop-shadow-md drop-shadow-black" />
                </div>

                <button onClick={() => isFavorite ? dispatch(removeFavorites(id)) : dispatch(addFavorites(id))} className={isFavorite ?
                    "flex col-span-6 lg:col-start-10 col-start-4 row-start-7 p-4 bg-red-500 rounded-2xl h-10 place-self-center place-items-center drop-shadow-lg drop-shadow-black" :
                    "flex col-span-6 lg:col-start-10 col-start-4 row-start-7 p-4 bg-yellow-300 rounded-2xl h-10 place-self-center place-items-center drop-shadow-lg drop-shadow-black"
                }>
